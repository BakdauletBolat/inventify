...
def import_warehouse_details():
    ...
    for pageNumber in range(page, 110, 1):
        ...
        for warehouse in warehouses:
            import_warehouse_detail.delay(warehouse['id'])


@shared_task
def import_warehouse_detail(warehouse_id):
    ImportWarehouseAction().import_detail(warehouse_id)
...
def import_warehouse():
    ...
    for pageNumber in range(page, 150, 1):
        warehouses = RecarRequest().get_warehouses(pageNumber, size)
        for warehouse in warehouses:
...
def import_warehouses_from_recar():
    ...
    time.sleep(300)
    import_warehouse_details.delay()
